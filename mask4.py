        ...
        for i in range(len(mask)):
            for j in range(len(mask[0])):
                    if (mask[i][j] == True):
                        ...
                        # pixel_img[i][j] = 255
        arr = np.array(arr)
        num_rows_in_sample = int(arr.shape[0] * 0.01)
        # while pothole_depth == 1e9:
        #     print('again')
        #     for point in arr:
        #         projection = coordinates.get_coordinate(13, point[0], point[1])
...
def perform_inference_video(use_camera, video_path, model, colors, classes, draw_bbox, mrcnn_visualize,
    ...
    while True:
        ...
        if save_enable and video_flag:
            out = cv2.VideoWriter("skip5.mp4", cv2.VideoWriter_fourcc(*'MP4V'), 24,
