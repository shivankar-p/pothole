...
def custom_visualize(test_image, model, colors, classes, draw_bbox, mrcnn_visualize, instance_segmentation, frame_no):
    ...
    for i in range(0, detections["rois"].shape[0]):
        ...
        for i in range(len(mask)):
            for j in range(len(mask[0])):
                    if (mask[i][j] == True):
                        ...
                        arr.append([i, j])
        loop_end = time.time()
        print("loop calculation time :" + str(loop_end-loop_start))

        perimeter_start = time.time()
        perimeter_pixels = depth_estimator.get_perimeter(mask, first_row, last_row)
        perimeter_end = time.time()

        print("perimeter calculation time :" + str(perimeter_end-perimeter_start))
        
        ref_depth = depth_estimator.get_ref_depth(perimeter_pixels, frame_no)
        ...
        for point in arr[np.random.choice(arr.shape[0], num_rows_in_sample, replace=False)]:
            projection = coordinates.get_coordinate(frame_no, point[0], point[1])
...
def perform_inference_video(use_camera, video_path, model, colors, classes, draw_bbox, mrcnn_visualize,
    ...
    while True:
        ...
        if save_enable and video_flag:
            out = cv2.VideoWriter("depth_vid_05.mp4", cv2.VideoWriter_fourcc(*'MP4V'), 24,
